...
class Floor:
    ...
    """
    def __init__(self, y_pos: int = 500):
        """
            Constructs all the necessary attributes for the floor object.
               Parameters:
    ...
    def move(self):
        ...
        """
        self.x = [pos - FloorConsts.FLOOR_VELOCITY for pos in self.x]  # move position left with respect to velocity
        if abs(self.x[0]) > abs(FloorConsts.FLOOR_IMAGE.get_width()):  # if the images move out of screen
            self.x[0] = self.x[3] + FloorConsts.FLOOR_IMAGE.get_width()
        ...
        if abs(self.x[0]) > abs(FloorConsts.FLOOR_IMAGE.get_width()):  # if the images move out of screen
            self.x[3] = self.x[2] + FloorConsts.FLOOR_IMAGE.get_width()
